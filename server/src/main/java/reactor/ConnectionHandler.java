 ...
 */
public class ConnectionHandler<T> implements MyHandler<String> {
	...
	private TBGPProtocolCallback<T> _protocolCallback = null;
	 ...
	 */
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, TBGPProtocolCallback<T> callback) {
		_sChannel = sChannel;
		_protocolCallback = callback;
		callback.setHandler(this);
		_data = data;
		_protocol = (AsyncServerProtocol<T>) _data.getProtocolMaker().create();
	...
	public  TBGPProtocolCallback<T> getCBProtocol(){
		return _protocolCallback;
		
	}
	
	public static <T> ConnectionHandler<T> create(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, TBGPProtocolCallback<T> callback) {

		ConnectionHandler<T> h = new ConnectionHandler<T>(sChannel, data, key, callback);
		h.initialize();
		return h;
	}
